...
function setTreeOpen(treeName, isOpen) {
  ...
  panel.hidden = !isOpen;
  const chevron = parent.querySelector(".side-chevron");
...
const linkedSectionIds = new Set(
  sideLinks.map((link) => link.getAttribute("href")?.replace("#", "")).filter((id) => Boolean(id)),
);
...
function buildAssistantAnswer(question) {
  ...
  const first = matches[0] ?? searchable[0];
  if (!first) {
    return "<p>No searchable documentation content is available.</p>";
  }
...
document.addEventListener("click", (event) => {
  ...
  if (treeParent) {
    ...
    const href = treeParent.getAttribute("href");
    if (href) navigateToHash(href);
    return;
  }
