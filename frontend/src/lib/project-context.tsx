...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [activeProject, design.activeId, design.conversations, design.setActiveId, design.setConversations]);
    ...
    try {
      ...
      if (isAuthError) {
        ...
        try {
          ...
          try { localStorage.removeItem("qs_token"); } catch { }
        }
      }
    ...
    try {
      ...
      setProjects(prev => prev.map(p =>
        p.id === activeProject.id
          ? { ...p, has_design: true, num_qubits: payload.num_qubits, topology: payload.topology, updated_at: now }
          : p
      ));
      _setActiveProject(prev =>
        prev?.id === activeProject.id
          ? { ...prev, has_design: true, num_qubits: payload.num_qubits, topology: payload.topology, design_payload: payload, updated_at: now }
          : prev
      );
    } catch {
