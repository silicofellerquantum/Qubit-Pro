  ...
  return (
    <>
      ...
      <p className="text-[var(--muted)] mb-8 leading-relaxed">
        Advanced parameters and finite-element mesh setup for Sim Eigenmode. This module provides researchers and hardware engineers with the necessary tooling to optimize their quantum design lifecycle within the Silicofeller platform. It has been built from the ground up to support massive scale, seamless AI integration via the Copilot, and absolute precision down to the nanometer level.
      </p>
      ...
      <p className="mb-6">
        The <strong>Sim Eigenmode</strong> sub-system is heavily integrated into the core QuantumChipGen pipeline. When designing superconducting layouts, maintaining precise control over this module ensures high-fidelity qubit operation, minimized cross-talk, and reduced dielectric loss tangents. The data structures are serialized via Protobuf before being handed off to the finite element solvers.
      </p>

      <AlertBox type="info" title="Best Practices">
        Always ensure that your QCLang definitions strictly adhere to the parameter constraints of Sim Eigenmode to prevent Design Rule Checking (DRC) violations during the final synthesis phase.
      </AlertBox>

      <h3 className="mt-10">Parameter Reference</h3>
      <p className="mb-4">The following table outlines the configuration schema available for <code>sim-eigenmode</code>. These properties can be manipulated directly via QCLang or the Python bindings.</p>
      ...
      <p className="mb-4">Below is a standard reference implementation used by the Silicofeller synthesis engine. It demonstrates how to initialize the Sim Eigenmode module correctly.</p>
      
      <CodeBlock language="qclang" code={`simulate sim-eigenmode {
  accuracy: 1e-6;
