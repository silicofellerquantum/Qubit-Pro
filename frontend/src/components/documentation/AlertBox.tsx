...
export function AlertBox({ type, title, className, children }: AlertBoxProps) {
  if (type === "warning") {
    return (
      <div
        className={cn(
          "warning-box my-[18px] mb-[26px] rounded-xl border border-amber-300 bg-amber-50 px-[18px] py-4 text-amber-900",
          className
        )}
      >
        {title && (
          <strong className="block mb-1.5 font-bold text-amber-900">
            {title}
          </strong>
        )}
