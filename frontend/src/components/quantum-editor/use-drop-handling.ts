...
export function useDropHandling(dispatch: Dispatch): DropHandlingResult {
  ...
  const snapAndConstrain = (raw: { x: number; y: number }, snap: number) => ({
    x: Math.max(
      -CHIP_HALF_W,
      Math.min(CHIP_HALF_W, parseFloat((Math.round(raw.x / snap) * snap).toFixed(3))),
    ),
    y: Math.max(
      -CHIP_HALF_H,
      Math.min(CHIP_HALF_H, parseFloat((Math.round(raw.y / snap) * snap).toFixed(3))),
    ),
  });
        ...
        .then((metaRes) => {
          if (metaRes.data) {
            dispatch({
              ...
              id: placementId,
              patch: { params: defaultParamsFromMetadata(metaRes.data) },
            });
          }
        })
        .catch(console.error);
