...
async def create_project(
    ...
    # ── Enforce Quotas ──
    count_result = await db.execute(select(func.count(Project.id)).where(Project.owner_id == user.id))
    ...
    if plan == "free":
        ...
        limit = 5
    elif plan in ["pro", "test_usd", "test_inr"]:
        limit = 10
    else:
        limit = 2
        
    if current_count >= limit:
        if plan in ["pro", "test_usd", "test_inr"]:
            raise HTTPException(status_code=403, detail="Project limit reached. You have created the maximum number of projects (10) for the Pro plan.")
        else:
            raise HTTPException(
                status_code=403, 
                detail=f"Project limit reached. You are on the {plan.capitalize()} plan which allows up to {limit} projects. Please upgrade your plan to create more projects."
            )
