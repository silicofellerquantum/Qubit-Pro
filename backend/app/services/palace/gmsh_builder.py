...
class GmshBuilder:
    ...
    def generate_mesh(geometry: EMGeometry, output_path: Path, coarse: bool = False) -> None:
        ...
        try:
            ...
            # 2. Terminals (Qubits pads and Resonators) - second priority
            curr_attr = 10
            for el in geometry.elements:
                if el.kind == GeometryElementKind.QUBIT:
                    ...
                    curr_attr += 1
                elif el.kind == GeometryElementKind.RESONATOR:
            ...
            # 3. PEC ground plane and passive traces - lowest priority
            ground_tags_raw = set()
            for g_tag in final_ground_tags:
                ground_tags_raw.update(get_output_tags(2, (2, g_tag)))

            for el in geometry.elements:
                if el.kind in (GeometryElementKind.COUPLER, GeometryElementKind.FEEDLINE, GeometryElementKind.LAUNCHPAD):
                    trace_tag = trace_map[el.id]
                    ground_tags_raw.update(get_output_tags(2, (2, trace_tag)))
            ...
            for attr, (tags, name) in port_groups.items():
                ...
                if unique_tags:
                    ...
                    logger.warning("Port '%s' (attr %d) has no unique surfaces!", name, attr)
            ...
            for attr, (tags, name) in terminal_groups.items():
                ...
                if unique_tags:
                    ...
                    logger.warning("Terminal '%s' (attr %d) has no unique surfaces!", name, attr)

            # Define PEC (lowest priority)
            pec_tags = ground_tags_raw - assigned_surfaces
            if pec_tags:
            ...
            # Force GMSH to respect sizing fields and suppress defaults
