...
class PalaceInductanceEntry(BaseModel):
    """Inductance value for a specific inductive element."""
    element_id: str = Field(description="Identifier for the inductive element.")
    inductance_nH: float = Field(description="Inductance value in nanohenries.")


class PalaceMagnetostaticOutput(BaseModel):
    """Lumped inductance matrix output parsed from simulation."""
    inductance_data: List[PalaceInductanceEntry] = Field(default_factory=list, description="List of inductance entries.")
...
class PalaceSimulationOutput(BaseModel):
    ...
    magnetostatic: Optional[PalaceMagnetostaticOutput] = None
