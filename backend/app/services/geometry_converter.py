...
class QiskitMetalToPalaceConverter:
    ...
    def _extract_from_dict(self, design_dict: dict) -> None:
        ...
        def parse_val(val_name: str, s: Any) -> float:
            if s is None:
                return 0.5
            if isinstance(s, (int, float)):
                val = float(s)
                if "um" in val_name.lower():
                    return val / 1000.0
                if val > 10.0:
                    return val / 1000.0
                return val
            s_str = str(s).strip().lower()
            if s_str.endswith("um"):
                return float(s_str[:-2]) / 1000.0
            if s_str.endswith("mm"):
                return float(s_str[:-2])
            try:
                val = float(s_str)
                if "um" in val_name.lower():
                    return val / 1000.0
        ...
        if graph_dict:
            ...
            for n in nodes:
                ...
                w = parse_val("pocket_width_um", width_str)
                h = parse_val("pocket_height_um", height_str)
            ...
            for pl in placements:
                ...
                w = parse_val("pocket_width", width_str)
                h = parse_val("pocket_height", height_str)
