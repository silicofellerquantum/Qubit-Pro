...
def test_gmsh_builder_generation(sample_geometry, tmp_path):
    ...
    # Run the generator
    GmshBuilder.generate_mesh(sample_geometry, msh_path, coarse=True)
...
async def test_palace_runner_uses_gmsh_builder(sample_geometry):
    ...
    mock_proc_run.stdout.readline = AsyncMock(side_effect=[b"Palace Solver run completed.", b""])
    ...
    with patch("app.services.palace.gmsh_builder.GmshBuilder.generate_mesh") as mock_gmsh, \
        ...
        def mock_generate_mesh(geometry, output_path, coarse=False):
