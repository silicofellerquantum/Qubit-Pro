...
async def test_simulations_router_run_palace(sample_v2_payload):
    ...
    # We need 4 return values: 2 for the router, 2 for the background task
    mock_db.execute.side_effect = [
        mock_sim_result,   # First query: Simulation lookup (in router)
        mock_proj_result,  # Second query: Project lookup (in router)
        mock_sim_result,   # Third query: Simulation lookup (in background task)
        mock_proj_result   # Fourth query: Project lookup (in background task)
    ]
    ...
    with patch("app.routers.simulations.settings.palace_mock_mode", True), \
         patch("app.routers.simulations.AsyncSessionLocal", return_value=MockSessionContext()):
        ...
        # Verify immediate response is running
        ...
        background_func = call_args.args[0]
        await background_func(**call_args.kwargs)
        
        # Verify simulation status updated in database
        assert mock_sim.status == SimulationStatus.completed
        assert mock_sim.runtime_seconds > 0
        assert mock_sim.results is not None
        assert "physics_score" in mock_sim.results
        assert "qubit_results" in mock_sim.results
...
async def test_palace_runner_missing_output_files():
    ...
    mock_proc_run.stdout.readline = AsyncMock(side_effect=[b"Simulation completed", b""])
    mock_proc_run.stderr.readline = AsyncMock(return_value=b"")
...
@pytest.mark.skip(reason="Obsolete: simulations are now enqueued asynchronously by the Phase 13 thin router and executed by the BackgroundWorker. Modern visualization and field image rendering tests are in test_visualization.py.")
@pytest.mark.asyncio
async def test_run_simulation_generates_mesh_image(sample_v2_payload):
    ...
    with patch("app.routers.simulations.settings.palace_mock_mode", True), \
        ...
        assert mock_sim.status == SimulationStatus.completed
        assert "field_images" in mock_sim.results
        assert len(mock_sim.results["field_images"]) > 0
